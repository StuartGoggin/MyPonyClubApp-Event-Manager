...
router.post('/', withAdminAuth(async (req: Request, res: Response, user: User) => {
  try {
    ...
    res.json({ success: true, id: newEmailId, message: 'Email added to queue successfully' });
 ...
 */
router.put('/', withAdminAuth(async (req: Request, res: Response, user: User) => {
  try {
 ...
 */
router.delete('/', withAdminAuth(async (req: Request, res: Response, user: User) => {
  try {
    const { id } = req.body;
