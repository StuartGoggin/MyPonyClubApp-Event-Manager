...
router.post('/', withAdminAuth(async (req: Request, res: Response, user: User) => {
  try {
    logger.info('Email Queue Send API: POST request started', { userEmail: user.email });
    
    const { emailId } = req.body;
    const sentById = user.email || user.id;
    
    logger.info('Email Queue Send API: Request details', { emailId, sentById });
    ...
    logger.info('Email Queue Send API: Fetching email from queue', { emailId });
    ...
    if (!process.env.RESEND_API_KEY || !resend) {
      logger.info('Email Queue Send API: RESEND_API_KEY not configured, simulating email send', {
        subject: email.subject,
