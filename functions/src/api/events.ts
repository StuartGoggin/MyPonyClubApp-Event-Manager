...
router.get("/", async (req, res) => {
  try {
    if (!adminDb || !isDatabaseConnected()) {
      const errorMessage =
        getDatabaseErrorMessage() || "Database connection not available";
      logger.warn("Events API: Database connection issue", {
        error: errorMessage,
      });
    ...
    if (zoneId) {
      ...
      const clubsSnapshot = await adminDb
        .collection("clubs")
        .where("zoneId", "==", zoneId)
        .get();
    ...
    snapshot.forEach((doc: any) => {
      ...
      try {
        if (data.date && typeof data.date.toDate === "function") {
          ...
          logger.warn("Events API: Invalid date format", {
            eventId: doc.id,
            date: data.date,
          });
        ...
        logger.error("Events API: Error converting date", {
          eventId: doc.id,
          error: error,
        });
        eventDate = new Date(); // Fallback to current date
...
router.post("/", async (req, res) => {
  try {
    if (!adminDb || !isDatabaseConnected()) {
      const errorMessage =
        getDatabaseErrorMessage() || "Database connection not available";
      logger.warn("Events API POST: Database connection issue", {
        error: errorMessage,
      });
      return res.status(503).json({
        error: "Database connection unavailable",
    ...
    logger.info("Events API: Creating new event", {
    ...
    if (isNaN(eventDate.getTime())) {
      logger.warn("Events API: Invalid date provided", {
        date: eventData.date,
      });
    ...
    logger.error("Events API: Error creating event", {
      error: error.message,
      stack: error.stack,
    });
    ...
    if (
      ...
      logger.warn(
        "Events API POST: Database connection timeout or unavailable",
      );
      return res.status(503).json({
        error: "Database connection timeout",
        message:
