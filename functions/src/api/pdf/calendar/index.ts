...
export async function generateCalendarPdfHandler(req: Request, res: Response) {
  try {
    ...
    logger.info('Calendar PDF: Request parameters', {
      scope, year: yearNum, month: monthNum, startDate, endDate, filterScope, zoneId, clubId
    });
    ...
    if (filterScope === 'zone' && zoneId) {
      ...
      filteredEvents = filteredEvents.filter((event: any) => event.clubId === clubId);
    }
    ...
    const enhancedEvents = filteredEvents.map((event: any) => {
      ...
      const eventType = eventTypes.find((et: any) => et.id === event.eventTypeId);
      ...
      return {
        ...
        location: event.location || club?.physicalAddress || club?.postalAddress,
    ...
    const filename = scope === 'month' ? `calendar_month_${yearNum}_${monthNum.toString().padStart(2, '0')}.pdf` :
                    scope === 'year' ? `calendar_year_${yearNum}.pdf` :
