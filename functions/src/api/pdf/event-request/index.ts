 ...
 */
export async function generateEventRequestPdfHandler(req: Request, res: Response) {
  try {
    logger.info('Event Request PDF generation started', { userAgent: req.get('User-Agent') });
    ...
    if (club) {
      ...
      logger.info('Event Request PDF: Club data enriched', { clubId: formData.clubId, clubName: club.name });
    }
