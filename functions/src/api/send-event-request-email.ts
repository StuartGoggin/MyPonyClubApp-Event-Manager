...
router.post('/', async (req, res) => {
  try {
    ...
    logger.info('Received data', { hasFormData: !!data.formData, hasPdfData: !!data.pdfData });
    ...
    if (approverEmails.length === 0) {
      logger.warn('No zone approver emails configured', { zoneName: zone.name, zoneCode });
    }
    ...
    logger.info('Preparing email for requesting user', { email: formData.submittedByEmail });
    ...
    if (shouldQueue) {
      ...
      const queuedEmailData: Omit<QueuedEmail, 'id' | 'createdAt' | 'updatedAt'> = {
      ...
      try {
        ...
        logger.info('Requester email sent successfully', { id: result.data?.id });
    ...
    for (const approverEmail of approverEmails) {
      logger.info('Preparing email for zone approver', { email: approverEmail });
      ...
      if (shouldQueue) {
        logger.info('Queueing email for zone approver', { email: approverEmail });
        ...
        try {
          logger.info('Sending immediate email to zone approver', { email: approverEmail });
          const result = await resend.emails.send(zoneApproverEmail);
          logger.info('Zone approver email sent successfully', { id: result.data?.id, approverEmail });
          emailResults.push({ type: 'zone-approver', success: true, id: result.data?.id, recipient: approverEmail });
        } catch (error) {
          logger.error('Failed to send zone approver email', { error, approverEmail });
    ...
    for (const superUserEmail of superUserEmails) {
      ...
      if (shouldQueue) {
        ...
        try {
          logger.info('Sending immediate email to super user', { email: superUserEmail });
          const result = await resend.emails.send(superUserEmailMessage);
          logger.info('Super user email sent successfully', { id: result.data?.id, superUserEmail });
          emailResults.push({ type: 'super-user', success: true, id: result.data?.id, recipient: superUserEmail });
        } catch (error) {
          logger.error('Failed to send super user email', { error, superUserEmail });
    ...
    logger.error('Email sending error', { error: error instanceof Error ? error.message : error });
