...
export async function addEmailToQueue(
  email: Omit<QueuedEmail, "id" | "createdAt" | "updatedAt">,
): Promise<string> {
  try {
    const emailData = {
      ...email,
...
export async function getQueuedEmailById(
  emailId: string,
...
export async function getEmailQueueStats(): Promise<EmailQueueStats> {
  try {
    ...
    const stats: EmailQueueStats = {
      ...
      sentToday: allEmails.filter(
        (e) => e.status === "sent" && e.sentAt && e.sentAt >= today,
      ).length,
      sentThisWeek: allEmails.filter(
        (e) => e.status === "sent" && e.sentAt && e.sentAt >= weekStart,
      ).length,
      sentThisMonth: allEmails.filter(
        (e) => e.status === "sent" && e.sentAt && e.sentAt >= monthStart,
      ).length,
    ...
    if (processedEmails.length > 0) {
      const totalProcessingTime = processedEmails.reduce((sum, email) => {
        const processingTime =
          email.sentAt!.getTime() - email.createdAt.getTime();
        return sum + processingTime;
      }, 0);
      stats.averageProcessingTimeMinutes = Math.round(
        totalProcessingTime / processedEmails.length / 60000,
      );
    }
...
export async function getEmailQueueConfig(): Promise<EmailQueueConfig> {
  try {
    const configDoc = await adminDb
      .collection(EMAIL_CONFIG_COLLECTION)
      .doc("default")
      .get();
...
export async function updateEmailQueueConfig(
