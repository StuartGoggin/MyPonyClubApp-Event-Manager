...
const serviceAccountStr = process.env.FIREBASE_SERVICE_ACCOUNT_KEY || (functions.config().service && functions.config().service.key);
...
if (isEmulator) {
  ...
  try {
    if (!admin.apps.length) {
      let serviceAccount = serviceAccountStr;
