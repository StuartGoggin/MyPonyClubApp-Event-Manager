...
export function validateUser(userData: Partial<User>): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  // Role validation
  if (userData.role && !['standard', 'zone_rep', 'super_user'].includes(userData.role)) {
...
export function validateLoginCredentials(ponyClubId: string, mobileNumber: string): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
