...
function generateFormPage(formData: EventRequestFormData): Buffer {
  try {
    ...
    // Intro text
    const line1 = 'The SMZ creates a calendar of events for the next calendar year according to the policy and procedures.';
    const line2 = 'Please submit your events for the following calendar year prior to the August Zone meeting/AGM.';
    const line3 = 'Events submitted via the MyPonyClub - Event Manager, Request Event will be submitted directly to the';
    ...
    doc.line(coordinatorFieldStart, yPosition + 1, coordinatorFieldEnd, yPosition + 1);
    ...
    doc.text('Preference', currentX + (colWidths[0]/2), yPosition + 9, { align: 'center' });
    currentX += colWidths[0];
    
    // Type of Event column  
    doc.rect(currentX, yPosition, colWidths[1], 14);
    doc.text('Type of Event', currentX + (colWidths[1]/2), yPosition + 9, { align: 'center' });
    currentX += colWidths[1];
    
    // Qualifier column
    doc.rect(currentX, yPosition, colWidths[2], 14);
    doc.setFontSize(9);
    doc.text('Will this be a', currentX + (colWidths[2]/2), yPosition + 6, { align: 'center' });
    doc.text('SMZ Qualifier', currentX + (colWidths[2]/2), yPosition + 11, { align: 'center' });
    doc.setFontSize(10);
    currentX += colWidths[2];
    
    // Event Date column
    doc.rect(currentX, yPosition, colWidths[3], 14);
    doc.text('Event Date', currentX + (colWidths[3]/2), yPosition + 9, { align: 'center' });
    ...
    for (let i = 0; i < maxEvents; i++) {
      ...
      if (event) {
        ...
        doc.text(`${event.priority}${getOrdinalSuffix(event.priority)}`, currentX + (colWidths[0]/2), yPosition + 13, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
      } else {
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(11);
        doc.text(`${i + 1}${getOrdinalSuffix(i + 1)}`, currentX + (colWidths[0]/2), yPosition + 13, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
      }
      currentX += colWidths[0];
      ...
      if (event) {
        ...
        doc.text(qualifierText, currentX + (colWidths[2]/2), yPosition + 13, { align: 'center' });
        doc.setTextColor(0, 0, 0); // Reset to black
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(10);
      }
      ...
      if (event) {
        ...
        dateLines.slice(0, 2).forEach((line: string, index: number) => {
          doc.text(line, currentX + (colWidths[3]/2), yPosition + 9 + (index * 4), { align: 'center' });
        });
        doc.setFontSize(10);
      }
    ...
    const availableSpaceForNotes = pageHeight - yPosition - fixedFooterHeight - whoFilledSectionHeight;
    ...
    if (formData.generalNotes) {
      doc.setFontSize(9);
      const noteLines = doc.splitTextToSize(formData.generalNotes, contentWidth - 4);
    ...
    doc.setFontSize(7);
    doc.setTextColor(255, 0, 0);
    const clubsText1 = 'CLUBS: BALNARRING – CHELSEA – DANDENONG RANGES – DOONGALA – HASTINGS – LANGWARRIN – MACCLESFIELD – MAIN RIDGE – MENTONE –';
    const clubsText2 = 'MERRICKS – MONASH – MONBULK – MORNINGTON PENINSULA – MOUNTAIN DISTRICT – PEARCEDALE';
    ...
    doc.setFontSize(12);
    doc.text('Unable to generate event request PDF at this time.', 105, 70, { align: 'center' });
    doc.text('Please contact support if this issue persists.', 105, 85, { align: 'center' });
...
function generateFallbackPDF(options: EventRequestPDFOptions): Buffer {
  try {
    ...
    doc.setFont('times', 'normal');
    doc.setFontSize(12);
    doc.text('Unable to load original policy document.', 105, 70, { align: 'center' });
    doc.text('Please contact support for assistance.', 105, 85, { align: 'center' });
