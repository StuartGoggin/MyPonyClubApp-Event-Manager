...
export class SpreadsheetParser {
  ...
  private static readonly EXPECTED_HEADERS = {
    ponyClubId: ['pony club id', 'ponyclubid', 'pc id', 'id', 'pcid', 'pony_club_id'],
    ...
    email: ['email', 'email address', 'e-mail', 'email_address', 'e_mail', 'mail']
  };
  ...
  private static parseRowData(rawData: string[][]): { data: UserImportRow[]; errors: string[] } {
    ...
    for (let i = 1; i < rawData.length; i++) {
      ...
      try {
        ...
        console.error(`[SpreadsheetParser] Error parsing row ${i + 1}:`, errorMessage);
  ...
  private static validateRequiredColumns(columnMapping: Record<string, number>): string[] {
    const requiredFields = ['ponyClubId', 'mobileNumber', 'clubName', 'zoneName', 'role'];
